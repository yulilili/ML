...
import operator
"""
利用信息增益比作为选择特征的方式 每次选择信息增益比较大的特征作为划分的依据，成为决策点
直到每个叶子节点上都只有同样标签的样本或者所有的特征全部已经考虑过了为止

信息增益比：信息增益/以该特征作为数据集确定性的依据计算所得的熵

和ID3唯一的区别就是这里用于选择特征的标准是信息增益比，进一步考虑了所选择的特征本身的确定性，
如果特征本身的确定性本来就很差，其本身取值就很多，那么即使该特征能够很好的划分数据，也没有很大的意义
"""

def Ent(dataset, index):
    """
    计算数据集的熵
    :param dataset: 训练数据集
    :param index: 用于计算熵的维度参数（依据哪一个维度的参数来判断数据集的不确定性）
                 一般计算都是依据标签，也就是取值为-1
...
def info_Gain_ratio(dataset, fea):
    """
    计算根据特征fea得到的信息增益比
    ...
    for k in feaDB.keys():
        ...
        current_Ent = float(l)/numEntries*Ent(data_k,-1)
        fea_Ent += current_Ent
    # 返回信息增益
    Add = ori_Ent-fea_Ent
    f_ent = Ent(dataset, fea)
    return Add/f_ent
...
def chooseBestFeaToSplit(dataset):
    ...
    for i in range(len(dataset[0])-1):
        curInfoGain = info_Gain_ratio(dataset, i)
